...
export default async function Song({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="archive-page song-page">
    ...
    <div className="song-body section-shell"><section className="full-video" id="listen"><p className="section-number">01 / Listen</p><h2>The complete song.</h2><div className="video-frame"><iframe src={`https://archive.org/embed/echo-of-humanity-${slug}`} title={`${item.title} — full song by Echo`} loading="lazy" allow="fullscreen" allowFullScreen /></div><div className="media-links"><a href={archiveUrl(slug)} target="_blank" rel="noreferrer">Watch or download on Internet Archive</a>{item.rumble && <a href={item.rumble} target="_blank" rel="noreferrer">Watch on Rumble</a>}{item.tiktok && <a href={`https://www.tiktok.com/@echoofhumanity7/video/${item.tiktok}`} target="_blank" rel="noreferrer">Watch on TikTok</a>}</div><p className="quiet-note">The player is provided by Internet Archive. If it does not load, use the Archive link above.</p></section>
    <div className="lyrics-layout"><section id="lyrics"><p className="section-number">02 / Lyrics</p><h2>{item.title}</h2>{slug === 'everybody-poops' && <div className="media-links"><a href="/archive/everybody-poops-timed-lyrics-2026-09-14.srt" download>Timed lyrics (SRT)</a><a href="/archive/everybody-poops-timed-lyrics-2026-09-14.vtt" download>Timed lyrics (VTT)</a></div>}<div className="song-lyrics">{songLyrics.split(/\n\s*\n/).map((stanza, i) => <p key={i}>{stanza.replace(/\*\*/g, '')}</p>)}</div></section><aside className="song-notes"><h2>About the song</h2><p>{songMeanings[slug]}</p><h2>About the recording</h2><p>{item.note}</p><h3>Creative process</h3><p>{recordingCredits[slug]}</p><p>{musicCreditPolicy}</p><p>The lyrics shown here are the supplied song text. Vocal phrasing and repeated lines in the recording may vary.</p><a href="/archive#music">Explore Before the Dawn</a><a href="/contact">Share feedback or a correction</a><a href="/support">Optional support</a></aside></div>
    <DiscussionInvitation title={item.title} rumble={item.rumble} tiktok={item.tiktok} />
