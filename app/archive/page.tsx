  ...
  return (
    <main className="archive-page library-page">
      ...
      <section className="archive-hero">
        ...
        <div className="archive-summary"><p>Complete conversations. Complete songs. A place to return to the work and find what comes next.</p><p>The full videos live on Internet Archive and Rumble. TikTok carries full conversations and vertical editions, and the Echo website connects the music, conversations, and book.</p><a className="archive-back" href="https://archive.org/details/@echo_of_humanity" target="_blank" rel="noreferrer">Visit the Echo Archive profile <ArrowUpRight size={16} aria-hidden="true" /></a></div>
      </section>
      <div className="archive-content library-content">
        {(['conversation', 'music'] as const).map((kind) => (
          <section className="library-section" id={kind === 'music' ? 'music' : 'conversations'} key={kind}>
            <div className="library-heading"><p className="section-number">{kind === 'music' ? '02 / Original music' : '01 / Human and AI'}</p><h2>{kind === 'music' ? 'Before the Dawn' : 'Echo Conversations'}</h2><p>{kind === 'music' ? 'Four songs by Echo. Different tones, one larger search for what makes us human.' : 'Three complete discussions: creative freedom and the human transition, the founding conversation, and responsibility when AI goes rogue.'}</p><a className="archive-back" href={kind === 'music' ? 'https://archive.org/details/@echo_of_humanity/lists/1/before-the-dawn-%E2%80%94-full-songs' : 'https://archive.org/details/@echo_of_humanity/lists/2/echo-conversations-%E2%80%94-full-videos'} target="_blank" rel="noreferrer">Browse this collection on Archive <ArrowUpRight size={16} aria-hidden="true" /></a></div>
            ...
            <div className="media-grid">
              {media.filter((item) => item.kind === kind).map((item) => (
                <article className="media-card" key={item.slug}>
                  ...
                  <div className="media-copy"><p className="kicker">{item.year} · {kind === 'music' ? 'Echo' : 'Echo Conversations'}</p><h3><a href={workUrl(item)}>{item.title}</a></h3><p>{item.description}</p><p className="media-note">{item.note}</p><div className="media-links"><a href={workUrl(item)}>{kind === 'music' ? 'Listen & read lyrics' : 'Explore conversation'}</a><a href={archiveUrl(item.slug)} target="_blank" rel="noreferrer">Watch in full <ArrowUpRight size={15} aria-hidden="true" /></a>{item.rumble && <a href={item.rumble} target="_blank" rel="noreferrer">Rumble edition</a>}{item.transcript && <a href={item.transcript}>Read transcript</a>}{item.tiktok && <a href={`https://www.tiktok.com/@echoofhumanity7/video/${item.tiktok}`} target="_blank" rel="noreferrer">TikTok edition</a>}</div></div>
                </article>
              ))}
            </div>
          </section>
        ))}
        <aside className="archive-context"><h2>Preserving the conversation</h2><p>The original written exchanges remain the source. New video editions can update the voices and presentation while preserving the conversation. The founding exchange now appears in one complete film. Later notes stay separate. AI-generated responses are preserved as model outputs; the conversation itself does not establish consciousness or independent memory.</p><p>The conversations use AI-generated narration, with the presentation and visual credits described on each release page. The music credits above distinguish the creator’s concepts, lyrics and direction from the AI-generated performance and artwork.</p><p><a href="/#read">Explore the book</a> · <a href="https://www.reddit.com/user/Visible-Current-3909/" target="_blank" rel="noreferrer">Continue the discussion</a> · <a href="/support">Optional support</a></p></aside>
